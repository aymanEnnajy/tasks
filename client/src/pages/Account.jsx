    ...
    return (
        <main className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-10 custom-scrollbar">
            <header>
                ...
                >
                    My <span className="text-zinc-400 italic">Account.</span>
                </motion.h2>
                <p className="text-zinc-500 font-black uppercase tracking-[3px] text-[10px]">Manage your profile and security</p>
            </header>

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                <div className="xl:col-span-2 space-y-8">
                    {/* Profile Section */}
                    <form onSubmit={handleSave} className="glass-panel p-6 sm:p-10 rounded-[30px] sm:rounded-[50px] border-white/5 space-y-8">
                        <div className="flex flex-col sm:flex-row items-center sm:items-center gap-6 sm:gap-8 mb-8 sm:mb-12 text-center sm:text-left">
                            <div className="relative group">
                                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-zinc-800 border-2 border-white/10 overflow-hidden shadow-2xl transition-all group-hover:border-zinc-500/50">
                                    <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${user?.email}`} alt="Avatar" />
                                </div>
                            </div>
                            <div>
                                <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">{formData.username || 'Engineer'}</h3>
                                <p className="text-zinc-400 font-bold text-xs sm:text-sm">System Role: {role || 'Standard User'}</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[3px] ml-1">Username</label>
                                <div className="relative group">
                                    <User className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-white" size={18} />
                                    <input
                                        value={formData.username}
                                        onChange={(e) => setFormData({ ...formData, username: e.target.value })}
                                        className="w-full bg-zinc-900 border border-white/5 rounded-2xl py-4 pl-12 pr-6 focus:ring-2 focus:ring-zinc-500 focus:bg-black outline-none transition-all text-white font-bold"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative opacity-60">
                                    ...
                                    <input
                                        ...
                                        disabled
                                        className="w-full bg-zinc-900 border border-white/5 rounded-2xl py-4 pl-12 pr-6 outline-none text-zinc-400 font-bold cursor-not-allowed"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="bg-zinc-700 hover:bg-zinc-600 text-white font-black py-4 px-8 rounded-2xl shadow-xl shadow-zinc-700/20 transition-all flex items-center gap-3 active:scale-95 text-[10px] uppercase tracking-widest disabled:opacity-50"
                        >
                    ...
                    <motion.div
                        ...
                        className="glass-panel p-6 sm:p-10 rounded-[30px] sm:rounded-[50px] border-white/5 space-y-8"
                    >
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <Lock size={20} className="text-zinc-400" />
                                <h3 className="text-lg sm:text-xl font-black text-white">Password Security</h3>
                            </div>
                            <button
                                type="button"
                                onClick={() => setShowPasswordSection(!showPasswordSection)}
                                className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white font-bold rounded-xl transition-all text-[9px] sm:text-[10px] uppercase tracking-widest w-full sm:w-auto"
                            >
                            ...
                            {showPasswordSection && (
                                ...
                                >
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="relative group">
                                            <Shield className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-white" size={18} />
                                            <input
                    ...
                    <motion.div
                        ...
                        className={`glass-panel p-6 sm:p-10 rounded-[30px] sm:rounded-[50px] border-white/5 space-y-6 sm:space-y-8 ${dbUserData?.deletion_scheduled_at ? 'border-red-500/30 bg-red-950/20' : ''}`}
                    >
                        {dbUserData?.deletion_scheduled_at ? (
                            <>
                                ...
                                <div className="bg-red-950/40 border border-red-500/30 rounded-3xl p-6 space-y-4">
                                    <p className="text-zinc-300 font-bold text-sm">Your account is scheduled for deletion. You have the following time to cancel:</p>
                                    
                                    {deletionCountdown && (
                                        <div className="grid grid-cols-3 gap-4">
                                            <div className="bg-black/80 rounded-2xl p-4 text-center border border-white/5">
                                                <div className="text-3xl font-black text-red-500">{deletionCountdown.days}</div>
                                                <div className="text-[10px] font-black text-zinc-500 uppercase tracking-[2px] mt-1">Days</div>
                                            </div>
                                            <div className="bg-black/80 rounded-2xl p-4 text-center border border-white/5">
                                                <div className="text-3xl font-black text-red-500">{deletionCountdown.hours}</div>
                                                <div className="text-[10px] font-black text-zinc-500 uppercase tracking-[2px] mt-1">Hours</div>
                                            </div>
                                            <div className="bg-black/80 rounded-2xl p-4 text-center border border-white/5">
                                                <div className="text-3xl font-black text-red-500">{deletionCountdown.minutes}</div>
                                                <div className="text-[10px] font-black text-zinc-500 uppercase tracking-[2px] mt-1">Minutes</div>
                                            </div>
                                        </div>
                                    )}
 
                                    <p className="text-zinc-400 text-sm font-bold mt-4">⚠️ All your tasks, articles, and account data will be permanently deleted after the countdown expires.</p>
                                </div>
                                <button
                                    type="button"
            ...
            <Modal isOpen={showDeleteModal} onClose={() => { setShowDeleteModal(false); setConfirmDeletePassword(''); }} title="Confirm Account Deletion">
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <p className="text-zinc-400 font-bold text-sm">To proceed, enter your password to confirm:</p>

                        <div className="space-y-3">
                            <div className="relative group">
                                <Shield className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-red-500" size={18} />
                                <input
                                    type={showDeletePassword ? 'text' : 'password'}
                                    value={confirmDeletePassword}
                                    onChange={(e) => setConfirmDeletePassword(e.target.value)}
                                    placeholder="Enter your password"
                                    className="w-full bg-black border border-white/5 rounded-2xl py-4 pl-12 pr-14 focus:ring-2 focus:ring-red-500 outline-none transition-all text-white font-bold placeholder:text-zinc-600"
                                />
                                <button
                                    type="button"
                                    onClick={() => setShowDeletePassword(!showDeletePassword)}
                                    className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-red-500 transition-colors"
                                >
